...
<html>
<head>
<title>課程資料 - listOneCour_Booking.jsp</title>
<link rel="stylesheet"
			...
			<div class="table-responsive">
				<table class="table table">
						...
						<th>${(cour_bookingVO.trainee_comf==1?'確認':(cour_bookingVO.trainee_comf==2?'未確認':'取消'))}
							<FORM METHOD="post"
								...
								<button type="button" class="btn btn-outline-danger" id="cancelbtn"
									style="float: right;"
									<c:if test="${cour_bookingVO.coa_comf == 3 || cour_bookingVO.trainee_comf}">
							disabled
						</c:if>>取消</button>
					...
					<tr class="table-danger">
						...
						<th>${(cour_bookingVO.trainee_comf==1?'已報到':'未報到')}</th>
					</tr>
					<tr class="table-success">
						<th scope="row">教練報到狀況</th>
						<th>${(cour_bookingVO.coa_comf==1?'已報到':'未報到')}</th>
					</tr>
					<tr class="table-danger">
						<th scope="row">評分</th>
