...
<%
	String prc_no = (String) session.getAttribute("check_prc");
	ProductService productSvc = new ProductService();
	List<ProductVO> list = productSvc.getShop(prc_no);
...
<head>
<meta charset="UTF-8">
<title>分類商品資料 - listCateShop.jsp</title>
			...
			<div class="col-md-2">
				...
				<span class="total_amount">
					<c:if test="${total_price > 0}">
						<a href="<%=request.getContextPath()%>/shopping.do?whichPage=<%=whichPage %>&action=getCart_Display&prc_no=<%=list.get(0).getPrc_no() %>">$${total_price}</a>
					</c:if>
				</span>
			</div>
		</div>
		...
		<div class="col-md-4 product">
			<div class="row justify-content-center" style="margin:10px auto 0px; display:table;">
				...
				<div class="col-md-10 pic offset-2" style="height: 250px;">
					<a href="<%=request.getContextPath()%>/shopping.do?whichPage=<%=whichPage %>&action=getOne_For_Display&pro_no=${productVO.pro_no}&prc_no=${productVO.prc_no}">
			...
			<div class="row justify-content-center">
				...
				<div class="col-md-10" style="text-align:center; margin:5px auto;">
					<a href="<%=request.getContextPath()%>/shopping.do?whichPage=<%=whichPage %>&action=getOne_For_Display&pro_no=${productVO.pro_no}&prc_no=${productVO.prc_no}">${productVO.pro_name}</a>			
				</div>
			</div>
			<div class="row">
				<div class="col-md">
			...
			<div class="row count justify-content-center" style="margin:10px auto;">
				...
				<div class="col-md">
					<FORM name="${productVO.pro_no}"  class="col-md">
						...
						<input class="btn btn-primary col-md-5 offset-1" type="button" name="shopping" value="放入購物車" style="margin-left:10px;"> 
	...
	$('[name=shopping]').click(function(){
		...
		$.ajax({
			...
			success: function(data){
				var url = "<%=request.getContextPath()%>/shopping.do?whichPage=<%=whichPage %>&action=getCart_Display&prc_no=<%=list.get(0).getPrc_no() %>";
